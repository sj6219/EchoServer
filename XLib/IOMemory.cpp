...
#ifdef	DEBUG_LEAK
	#if	defined(_WIN64)
		#define _MEM_TOP_DOWN	MEM_TOP_DOWN
		#define	HeapDestroy(hHeap)	TRUE
		#define HeapCreate(fOptions, dwInitailSize, dwMaximumSize)	((HANDLE) 1)
		#define HeapAlloc(hHeap, dwFlags, dwBytes) (LPVOID) VirtualAlloc(0, dwBytes, MEM_COMMIT | MEM_RESERVE | MEM_TOP_DOWN, PAGE_READWRITE)
		#define HeapFree(hHeap, dwFlags, lpMem)	VirtualFree(lpMem, 0, MEM_RELEASE)
		#define HeapValidate(hHeap, dwFlags, lpMem) TRUE
		#define HeapReAlloc(hHeap, dwFlags, lpMem, dwBytes)	_HeapReAlloc(hHeap, dwFlags, lpMem, dwBytes, _msize(lpMem))
		LPVOID	_HeapReAlloc(HANDLE hHeap, DWORD dwFlags, LPVOID lpMem, SIZE_T dwBytes, SIZE_T dwOldBytes)
		{
			LPVOID	lpNew = HeapAlloc(hHeap, dwFlags, dwBytes);
			memcpy(lpNew, lpMem, min(dwOldBytes, dwBytes));
...
#ifndef	_MEM_TOP_DOWN
#define _MEM_TOP_DOWN	0
#endif
...
XIOMemory::~XIOMemory()
{
#if	  defined(DEBUG_LEAK) && !defined(_WIN64)
	{
		HeapLock(_crtheap);
...
XIOMemory::PHEADER XIOMemory::__sbh_alloc_new_region()
{
	...
	{
#if	defined(DEBUG_LEAK) && defined(_WIN64)
		if (!(pHeader = (PHEADER)_HeapReAlloc(_crtheap, 0, __sbh_pHeaderList,
							(__sbh_sizeHeaderList + 16) * sizeof(HEADER), __sbh_sizeHeaderList * sizeof(HEADER))))
			return NULL;
#else
	...
	if ((pHeader->pHeapData = VirtualAlloc(0, BYTES_PER_REGION,
									 MEM_RESERVE | _MEM_TOP_DOWN, PAGE_READWRITE)) == NULL)
	{
